...
BarGaugeWidget::BarGaugeWidget(const Config& cfg, QWidget* parent)
...
void BarGaugeWidget::mousePressEvent(QMouseEvent* ev) {
...
void BarGaugeWidget::mouseMoveEvent(QMouseEvent* ev) {
...
void BarGaugeWidget::contextMenuEvent(QContextMenuEvent* ev) {
...
void BarGaugeWidget::dragEnterEvent(QDragEnterEvent* ev) {
...
void BarGaugeWidget::dropEvent(QDropEvent* ev) {
    auto from_id = ev->mimeData()->data("application/x-gauge-widget-id").toStdString();
    if (swap_cb_ && from_id != widget_id_) swap_cb_(from_id, widget_id_);
    ev->acceptProposedAction();
}

void BarGaugeWidget::paintEvent(QPaintEvent*) {
    QPainter p(this);
