...
LedGaugeWidget::LedGaugeWidget(const Config& cfg, QWidget* parent)
...
void LedGaugeWidget::mousePressEvent(QMouseEvent* ev) {
...
void LedGaugeWidget::mouseMoveEvent(QMouseEvent* ev) {
...
void LedGaugeWidget::contextMenuEvent(QContextMenuEvent* ev) {
...
void LedGaugeWidget::dragEnterEvent(QDragEnterEvent* ev) {
...
void LedGaugeWidget::dropEvent(QDropEvent* ev) {
    auto from_id = ev->mimeData()->data("application/x-gauge-widget-id").toStdString();
    if (swap_cb_ && from_id != widget_id_) swap_cb_(from_id, widget_id_);
    ev->acceptProposedAction();
}

void LedGaugeWidget::paintEvent(QPaintEvent*) {
    QPainter p(this);
