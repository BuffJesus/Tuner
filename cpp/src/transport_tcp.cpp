...
#else
// POSIX TCP stub.
void TcpTransport::open() { throw std::runtime_error("TCP not implemented on this platform"); }
void TcpTransport::close() {}
std::vector<std::uint8_t> TcpTransport::read(std::size_t, double) { return {}; }
std::size_t TcpTransport::write(const std::uint8_t*, std::size_t) { return 0; }
bool TcpTransport::is_open() const { return false; }
void TcpTransport::clear_buffers() {}
void TcpTransport::write_framed(const std::uint8_t*, std::size_t) {}
std::vector<std::uint8_t> TcpTransport::read_framed(double) { return {}; }
std::vector<std::uint8_t> TcpTransport::recv_exactly(std::size_t, double) { return {}; }
#endif
