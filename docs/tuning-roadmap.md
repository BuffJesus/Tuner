...
| 1 | **Diff on Connect** — reads all ECU pages on connect (33ms for 15 pages), compares scalars + tables, paged side-by-side dialog grouped by ECU page with per-page Keep Project / Keep ECU resolution. Remaining: side-by-side heatmap rendering for table diffs (currently shows min/max/avg summary), verify table staging doesn't corrupt row order | TS `G/an.java` + `U/h.java` | Medium | In progress |
...
| Bench simulator (Ardu-stim host) | **v1 done (Phase 17 Slices A–E)** | Backend: `bench_simulator::{wheel_pattern_catalog,config_codec,protocol,controller}` — 99 doctest cases. UI: `cpp/app/tabs/triggers_simulate_panel.{hpp,cpp}` Qt panel wired into TRIGGERS tab. Slice F SETUP wizard cross-link + Slice G docs + Slice E.1 polish queued |
...
| E | TRIGGERS-tab "Simulate" sub-panel — separate-from-ECU serial port picker, cylinder-filter chip strip (4/6/8/10/12), pattern picker with Speeduino/RusEFI badges, fixed/sweep RPM control, **compression-cycle controls** (enable toggle + cyl-type 2/4/6/8 dropdown + target RPM + offset + dynamic flag), signal-invert toggles (primary/secondary), live waveform preview via existing `TriggerScopeWidget` driven by the `P` pattern dump | Medium | ✅ **v1 done (2026-05-11).** `cpp/app/tabs/triggers_simulate_panel.{hpp,cpp}` (~520 LOC). Wired into `build_triggers_tab`. Surfaces: port picker + Connect/Disconnect, firmware-version chip (v1/v2 detect via `read_config`), cylinder filter strip, pattern picker with decoder badges, fixed/sweep RPM, compression simulator (v2-only enable), save to EEPROM, 500ms live RPM polling. **Slice E.1 follow-ups deferred:** surface bare-catch errors in UI, move polling reads off UI thread, bounds-check firmware-reported wheel index, hide (not just disable) compression card on v1, signal-invert toggles (firmware callbacks declared but no serial command yet — would need firmware contribution), live waveform preview via `P` dump |
...
**Current Linux/macOS status (2026-05-11):** Slices 1, 2, 3, 6, 7 done in one session. The full **Linux live-connect path is code-complete** — termios serial, /dev port enumeration, BSD-socket TCP, UDP discovery, GitHub Actions CI. Operator on Linux can pick a port from the dropdown, connect to a Speeduino over USB serial, or use Airbear TCP/UDP discovery → bench-test with Ardu-stim → tune the ECU end-to-end, all without typing paths manually. Slices 4 (CMake audit) + 5 (flash tool bundle) + 8–10 (arm64, macOS distribution, AppImage) remain for full polish.

| # | Slice | Effort | Notes |
|---|-------|--------|-------|
| 1 | Linux `SerialTransport` — replace the `#else` stub in `transport_serial.cpp` with a real `termios`-based POSIX implementation. Mirror the Win32 shape: synchronous reads with `VMIN`/`VTIME` for timeout, DTR/RTS forced low, baud-rate change via `tcsetattr` + `cfsetispeed`/`cfsetospeed`. Standard ~80 LOC | Small | ✅ **Done (2026-05-11).** ~250 LOC POSIX `#else` branch. macOS-specific `IOSSIOSPEED` ioctl for baud above 230400 (Bxxx constants max out at B230400 on macOS). `CLOCK_MONOTONIC` + `select()` for read timeouts. `TIOCEXCL` advisory exclusive access. DTR/RTS cleared via `TIOCMSET`. Header member conditional: `void* handle_` on Win32, `int fd_` on POSIX. Win32 build untouched (1601 / 12249 still passing). Validation gate: Phase 20 slice 7 Linux CI |
| 2 | Linux `list_serial_ports` — in `cpp/app/main.cpp`, swap the Windows registry walk for a `/dev/ttyUSB*` + `/dev/ttyACM*` glob. Detect names + sort. Stay behind `#ifdef _WIN32` / `#else` so the Win32 path is unaffected | Small | ✅ **Done (2026-05-11).** `dirent.h` scan over `/dev` with four prefix matches: `ttyUSB`, `ttyACM` (Linux), `cu.usbserial`, `cu.usbmodem` (macOS). Bluetooth dial-in skipped |
| 3 | Linux `TcpTransport` — winsock2 paths in `transport_tcp.cpp` (`SOCKET`, `closesocket`, `WSAStartup`, `select`-with-`fd_set`) need POSIX-socket alternatives. Most of the framing logic is portable; just the OS handles need conditional definitions | Small | ✅ **Done (2026-05-11).** Full BSD-socket implementation. SO_NOSIGPIPE on macOS (lacks MSG_NOSIGNAL). SO_ERROR check after select-on-connect catches ECONNREFUSED that the Win32 path silently misses. EINTR retry in `recv_exactly`. Distinguishes peer-closed from timeout in exception text |
...
| 6 | Linux UDP discovery — `udp_discovery.cpp` uses winsock2; needs POSIX socket parity. Same shape as slice 3 | Small | ✅ **Done (2026-05-11).** Was previously an unresolved-symbol link error on Linux/macOS (no `#else` branch existed); now full POSIX-socket equivalent with EINTR retry + EAGAIN backoff |
| 7 | Linux build CI — GitHub Actions job: Ubuntu 22.04 + Qt 6.6 from `apt` + `cmake --build` + `ctest`. Validates the test-only build at first; full Qt app build added after slices 1–6 land | Small | ✅ **Done (2026-05-11).** `.github/workflows/linux-build.yml` — Ubuntu 22.04, installs `qt6-base-dev`, vendors single-header deps, builds `tuner_core_tests` (correctness gate) + `tuner_app` (link check). Real validation gate for slices 1–6 (no display server in CI, no real ECU hardware) |
