...
def test_wideband_reference_table_checklist_replaces_missing_scalar_warning() -> None:
    app = QApplication.instance() or QApplication([])
    definition = EcuDefinition(
        name="Speeduino",
        scalars=[
    ...
    assert "Wideband calibration parameter not found" not in checklist
    assert "Wideband calibration parameter not in definition" not in checklist
    assert "All items OK" in checklist
...
def test_resolve_afr_table_name_prefers_target_table_over_generic_lambda_match() -> None:
    definition = EcuDefinition(
        name="Speeduino",
        tables=[
            TableDefinition(name="lambdaSensorData", rows=1, columns=2, page=1, offset=0),
            TableDefinition(name="afrTable1", rows=2, columns=2, page=2, offset=16),
        ],
        table_editors=[
            TableEditorDefinition(
                table_id="afr1",
    ...
    wizard = _wizard_with_tune(
        TuneFile(
            constants=[
                TuneValue(name="lambdaSensorData", value=[1.0, 1.0], rows=1, cols=2),
            ]
        ),
        definition=definition,
    )

    assert wizard._resolve_afr_table_name() == "afrTable1"
...
def test_generate_ve_table_uses_pending_induction_topology_draft() -> None:
    app = QApplication.instance() or QApplication([])
    definition = EcuDefinition(
        name="Speeduino",
        tables=[
    ...
    assert "topology: twin_turbo_identical" in status
    assert "topology: single_turbo" not in status
